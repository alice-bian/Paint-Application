...
type gctx = {
  ...
  thickness: thickness; (** the current thickness of pen *)
}
...
(** Produces a new Gctx.t with thickness. *)
let with_thickness (g: gctx) (t: thickness) : gctx =
  { g with thickness = t }

(** Set the OCaml graphics library's internal state so that it
agrees with the Gctx settings. Also sets current thickness for pen. *)
...
(** Draw a point at a specified position. *)
...
(** Draw all points in a list of points. *)
let draw_points (g: gctx) (p: position list) : unit =
  set_graphics_state g;
  let f (pos: position) : unit =
    draw_point g pos in
  List.iter f p
